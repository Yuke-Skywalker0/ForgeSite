...
const providerInfo: Record<
  BackendProvider,
  { label: string; tagline: string; bestFor: string; signupUrl: string }
> = {
  supabase: {
    label: "Supabase",
...
export function BackendSetupPanel({ projectId }: { projectId: string }) {
  ...
  if (isLoading) {
    return <p className="text-sm text-forge-text-muted">Caricamento configurazione…</p>;
  }
    ...
    return (
      <Card className="flex flex-col gap-3">
        <div className="flex items-center justify-between">
          <h3 className="font-display text-sm font-medium">Backend del sito</h3>
          <Badge
            variant={
              backend.status === "active"
                ? "success"
                : backend.status === "provisioning"
                  ? "warning"
                  : "danger"
            }
          >
        ...
        {backend.apiBaseUrl && (
          <a
            href={backend.apiBaseUrl}
            target="_blank"
            rel="noreferrer"
            className="flex items-center gap-1 text-xs text-forge-ember-soft hover:underline"
          >
            {backend.apiBaseUrl}
        ...
        <div className="flex gap-2">
          {backend.features.authEnabled && <Badge variant="ember">Auth</Badge>}
          {backend.features.databaseEnabled && <Badge variant="ember">Database</Badge>}
          {backend.features.customEndpoints && <Badge variant="ember">API custom</Badge>}
        </div>
      </Card>
    );
  }
  ...
  return (
    <Card className="flex flex-col gap-5">
      <div>
        <h3 className="font-display text-sm font-medium">Aggiungi un backend al sito</h3>
        <p className="text-sm text-forge-text-secondary">
          Facoltativo. Serve solo se il sito ha bisogno di login utenti, database, o logica
          server-side oltre ai form statici.
        </p>
      </div>
          ...
          {(Object.keys(providerInfo) as BackendProvider[]).map((key) => {
            ...
            return (
              <button
                ...
                className="flex flex-col items-start gap-2 rounded-md border border-forge-border bg-forge-surface-raised p-4 text-left hover:border-forge-ember/40"
              >
                <Icon size={18} className="text-forge-ember-soft" strokeWidth={1.75} />
                <span className="font-display text-sm font-medium">{info.label}</span>
      ...
      {provider && !accountMode && (
        <div className="flex flex-col gap-3">
          <p className="text-sm text-forge-text-secondary">
            Hai già un account {providerInfo[provider].label}?
          </p>
          <div className="flex gap-3">
            <Button variant="secondary" onClick={() => setAccountMode("existing")}>
              Sì, collega account esistente
            </Button>
            <Button variant="secondary" onClick={() => setAccountMode("new")}>
              No, voglio crearne uno
            </Button>
          </div>
          <Button variant="ghost" onClick={() => setProvider(null)} className="self-start">
            Cambia provider
          </Button>
        </div>
      )}
      ...
      {provider && accountMode === "new" && (
        <div className="flex flex-col gap-3 rounded-sm border border-dashed border-forge-border p-4">
          <p className="text-sm text-forge-text-secondary">
            Crea un account gratuito su {providerInfo[provider].label}, poi torna qui per
            collegarlo.
          </p>
          <a
          ...
          <Button variant="ghost" onClick={() => setAccountMode("existing")} className="self-start">
            Fatto, ora collego le chiavi
          </Button>
        </div>
      )}
      ...
      {provider && accountMode === "existing" && (
        <div className="flex flex-col gap-3">
          ...
          <p className="text-xs text-forge-text-muted">
            Le chiavi vengono cifrate at-rest e non sono mai visibili nel frontend dopo il
            salvataggio.
          </p>
          ...
          <div className="mt-2 flex gap-3">
            <Button variant="ghost" onClick={() => setAccountMode(null)}>
              Torna indietro
            </Button>
            <Button
              ...
              onClick={() =>
                provision.mutate({
                  ...
                  existingCredentials: { apiKey, projectUrl },
                  features,
                })
              }
            >
